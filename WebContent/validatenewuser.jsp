...
<%!
	String validatenewuser(JspWriter out,HttpServletRequest request, HttpSession session) throws IOException
	{
		...
		{
			getConnection();
			String verifyuserid = "SELECT customerid FROM customer WHERE userid = ?";
			PreparedStatement verifyuserId = con.prepareStatement(verifyuserid);
			verifyuserId.setString(1,username);
			ResultSet rst = verifyuserId.executeQuery();
			if(!rst.next()){
				...
				session.setAttribute("createMessage", "Username already in use. Please choose a different one.");
				return null;
			}
